...
RESULTS_DIR=/home/arch/Desktop/shared/optim/mcf
mkdir -p $RESULTS_DIR
 
# Fixed associativities
L1_INST_ASSOC=2         # L1 Instruction Cache Associativity
L1_DATA_ASSOC=2         # L1 Data Cache Associativity
L2_ASSOC=8              # L2 Cache Associativity
 
# Cache parameter values to vary
L1_INST_SIZES=("32kB" "64kB")  # L1 Instruction Cache Sizes
L1_DATA_SIZES=("64kB" "128kB") # L1 Data Cache Sizes
L2_SIZES=("2MB" "4MB")         # L2 Cache Sizes
CACHE_LINES=(64 128)           # Cache Line Sizes
 
# Benchmark-specific settings
BENCHMARK="specmcf"
BINARY_PATH="spec_cpu2006/429.mcf/src"
    ...
    for L1_DATA_SIZE in "${L1_DATA_SIZES[@]}"; do
        for L2_SIZE in "${L2_SIZES[@]}"; do
            for CACHE_LINE in "${CACHE_LINES[@]}"; do
                ...
                CONFIG_DIR="$RESULTS_DIR/l1i${L1_INST_SIZE}_l1d${L1_DATA_SIZE}_l2${L2_SIZE}_line${CACHE_LINE}_cpu${CPU_CLOCK}"
                mkdir -p $CONFIG_DIR
                ...
                -o \"${INPUT_FILE}\" -I 100000000"
 
                # Echo the command being run
                echo "Running benchmark: $BENCHMARK with L1I=${L1_INST_SIZE}, L1D=${L1_DATA_SIZE}, L2=${L2_SIZE}, Cache Line=${CACHE_LINE}, CPU Clock=${CPU_CLOCK}"
